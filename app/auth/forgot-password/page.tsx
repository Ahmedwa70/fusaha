...
export default async function ForgotPasswordPage() {
  ...
  const tForgotPassword = await getTranslations("auth.forgotPassword");
      ...
      <div className="flex flex-1 items-center justify-center px-6 py-16">
        <div className="motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-2 w-full max-w-sm motion-safe:duration-700">
          <div className="mb-8 space-y-2 text-center lg:text-start">
            <h2 className="font-heading text-2xl font-semibold">{tForgotPassword("title")}</h2>
            <p className="text-sm text-muted-foreground">{tForgotPassword("description")}</p>
          </div>
          <ForgotPasswordForm />
        </div>
      </div>
